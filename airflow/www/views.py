...
dagbag = None
fastdagbag = models.FastDagBag(settings.DAGS_FOLDER)
...
class Airflow(AirflowViewMixin, BaseView):
    ...
    def pickle_info(self):
        ...
        dag_id = request.args.get('dag_id')
        if not dag_id:
            raise Exception("Missing dag_id parameter")
        dag = fastdagbag.get_dag(dag_id)
    ...
    def action_set_failed(self, ids, session=None):
        try:
            ...
            for dr in session.query(DR).filter(DR.id.in_(ids)).all():
                ...
                count += 1
                altered_tis += \
                    set_dag_run_state_to_failed(fastdagbag.get_dag(dr.dag_id),
                                                dr.execution_date,
                                                commit=True,
                                                session=session)
    ...
    def action_set_success(self, ids, session=None):
        try:
            ...
            for dr in session.query(DR).filter(DR.id.in_(ids)).all():
                ...
                count += 1
                altered_tis += \
                    set_dag_run_state_to_success(fastdagbag.get_dag(dr.dag_id),
                                                 dr.execution_date,
                                                 commit=True,
                                                 session=session)
            altered_ti_count = len(altered_tis)
    ...
    def after_model_change(self, form, dagrun, is_created, session=None):
        ...
        if dagrun.state == State.SUCCESS:
            altered_tis = set_dag_run_state_to_success(
                fastdagbag.get_dag(dagrun.dag_id),

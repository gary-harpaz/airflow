...
dagdict = models.DagDict(settings.DAGS_FOLDER)
    ...
    def action_set_failed(self, ids, session=None):
        try:
            ...
            for dr in session.query(DR).filter(DR.id.in_(ids)).all():
                ...
                count += 1
                altered_tis += \
                    set_dag_run_state_to_failed(dagdict.get_dag(dr.dag_id),
                                                dr.execution_date,
                                                commit=True,
                                                session=session)
    ...
    def action_set_success(self, ids, session=None):
        try:
            ...
            for dr in session.query(DR).filter(DR.id.in_(ids)).all():
                ...
                count += 1
                altered_tis += \
                    set_dag_run_state_to_success(dagdict.get_dag(dr.dag_id),
                                                 dr.execution_date,
                                                 commit=True,
                                                 session=session)
            altered_ti_count = len(altered_tis)
